...
#
# Arguments:
#   project-type: outer | inner | both (default: both)
#
# Prerequisites:
...
case "$PROJECT_TYPE" in
  outer|inner|both)
    ;;
  *)
    echo -e "${YELLOW}Unknown project type: $PROJECT_TYPE, defaulting to both${NC}"
    PROJECT_TYPE="both"
    ;;
esac
...
write_prd_inner_section() {
  cat << 'EOF'

### Inner Gateway Guidelines

| File | What to Document |
|------|------------------|
| `.trellis/spec/inner/directory-structure.md` | Where different file types go (handlers, dblayer, k8s) |
| `.trellis/spec/inner/database-guidelines.md` | Database operations, query patterns, naming conventions |
| `.trellis/spec/inner/error-handling.md` | How errors are caught, logged, and returned |
| `.trellis/spec/inner/logging-guidelines.md` | Log levels, format, what to log |
| `.trellis/spec/inner/quality-guidelines.md` | Code review standards, testing requirements |
EOF
}

write_prd_outer_section() {
  cat << 'EOF'

### Outer Gateway Guidelines

| File | What to Document |
|------|------------------|
| `.trellis/spec/outer/directory-structure.md` | Component/page/hook organization |
| `.trellis/spec/outer/component-guidelines.md` | Component patterns, props conventions |
| `.trellis/spec/outer/hook-guidelines.md` | Custom hook naming, patterns |
| `.trellis/spec/outer/state-management.md` | State library, patterns, what goes where |
| `.trellis/spec/outer/type-safety.md` | TypeScript conventions, type organization |
| `.trellis/spec/outer/quality-guidelines.md` | Linting, testing, accessibility |
EOF
}
...
write_task_json() {
  ...
  case "$project_type" in
    outer)
      subtasks='[
    {"name": "Fill outer guidelines", "status": "pending"},
    ...
    inner)
      subtasks='[
    {"name": "Fill inner guidelines", "status": "pending"},
    {"name": "Add code examples", "status": "pending"}
  ]'
      related_files='[
    ".trellis/spec/inner/"
