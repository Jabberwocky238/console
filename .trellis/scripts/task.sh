...
get_implement_inner() {
  cat << EOF
{"file": "$DIR_WORKFLOW/$DIR_SPEC/inner/index.md", "reason": "Inner gateway development guide"}
{"file": "$DIR_WORKFLOW/$DIR_SPEC/inner/api-module.md", "reason": "API module conventions"}
{"file": "$DIR_WORKFLOW/$DIR_SPEC/inner/quality.md", "reason": "Code quality requirements"}
EOF
}

get_implement_outer() {
  cat << EOF
{"file": "$DIR_WORKFLOW/$DIR_SPEC/outer/index.md", "reason": "Outer gateway development guide"}
{"file": "$DIR_WORKFLOW/$DIR_SPEC/outer/components.md", "reason": "Component conventions"}
EOF
}
...
get_check_context() {
  ...
  local check_inner=$(get_command_path "check-inner")
  local check_outer=$(get_command_path "check-outer")
...
EOF
  ...
  if [[ "$dev_type" == "inner" ]] || [[ "$dev_type" == "both" ]]; then
    echo "{\"file\": \"${check_inner}\", \"reason\": \"Inner gateway check spec\"}"
  fi
  if [[ "$dev_type" == "outer" ]] || [[ "$dev_type" == "both" ]]; then
    echo "{\"file\": \"${check_outer}\", \"reason\": \"Outer gateway check spec\"}"
  fi
}
...
cmd_init_context() {
  ...
  if [[ -z "$target_dir" ]] || [[ -z "$dev_type" ]]; then
    ...
    echo "  dev_type: inner | outer | both | test | docs"
  ...
  local implement_file="$target_dir/implement.jsonl"
  {
    get_implement_base
    case "$dev_type" in
      inner|test) get_implement_inner ;;
      outer) get_implement_outer ;;
      both)
        get_implement_inner
        get_implement_outer
        ;;
...
cmd_create_pr() {
  ...
  local commit_prefix
  case "$dev_type" in
    feature|outer|inner|both) commit_prefix="feat" ;;
...
Arguments:
  dev_type: inner | outer | both | test | docs
...
Examples:
  ...
  $0 init-context .trellis/tasks/01-21-add-login inner
  $0 init-context .trellis/tasks/01-21-add-login outer --platform cursor
  $0 add-context <dir> implement .trellis/spec/inner/auth.md "Auth guidelines"
