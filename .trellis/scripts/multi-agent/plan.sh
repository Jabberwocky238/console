...
Arguments:
  ...
  --type, -t        Development type: inner | outer | both
  --requirement, -r Requirement description (quote if contains spaces)

Examples:
  $0 --name user-auth --type inner --requirement "Add JWT-based user authentication"
  $0 -n rate-limit -t inner -r "Add rate limiting to API endpoints"
...
if [[ ! "$DEV_TYPE" =~ ^(inner|outer|both)$ ]]; then
  log_error "Invalid dev type: $DEV_TYPE (must be: inner, outer, both)"
  exit 1
fi
